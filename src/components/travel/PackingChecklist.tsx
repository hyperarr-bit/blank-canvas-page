...
import { Plus, X, Package, Sparkles, Link2 } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {lists.map(l => {
          ...
          return (
            <button
              ...
              className={`shrink-0 rounded-2xl px-4 py-3 border transition-all text-left min-w-[140px] ${
                activeList === l.id
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className="flex-1 h-1 rounded-full bg-muted overflow-hidden">
                  <div className="h-full rounded-full bg-accent transition-all" style={{ width: `${total > 0 ? (packed / total) * 100 : 0}%` }} />
                </div>
                <span className="text-[9px] text-muted-foreground">{packed}/{total}</span>
              </div>
            </button>
          );
        })}
        <button
          ...
          className="shrink-0 rounded-2xl px-4 py-3 border border-dashed border-border hover:border-accent/50 min-w-[120px] flex items-center justify-center gap-1 text-muted-foreground text-xs transition-colors"
        >
      ...
      {/* Create form */}
      {showCreate && (
        <div className="rounded-2xl border border-accent/30 bg-card p-4 space-y-4">
          <Input
      ...
      {current && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-sm">{current.tripName}</h3>
              <p className="text-[10px] text-muted-foreground">
                {current.items.filter(i => i.packed).length} de {current.items.length} empacotados
              </p>
            </div>
            <Button variant="ghost" size="sm" className="text-destructive text-xs h-7" onClick={() => deleteList(current.id)}>
              Excluir
            </Button>
          </div>
          ...
          {grouped.map(([category, items]) => (
            <div key={category} className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className="p-2 space-y-0.5">
                {items.map(item => (
                  <div key={item.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-muted/30 group transition-colors">
                    <Checkbox
                      checked={item.packed}
                    ...
                    {item.linkedToSkincare && (
                      <span title="Pode vincular ao Inventário de Skincare" className="text-[9px] text-accent flex items-center gap-0.5">
