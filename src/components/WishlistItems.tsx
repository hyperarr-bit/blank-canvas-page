...
import { Plus, Trash2, ShoppingCart, TrendingUp, Calendar, Star, Heart } from "lucide-react";
...
export const WishlistItems = ({ items, setItems, monthlyBudget }: WishlistItemsProps) => {
  ...
  const monthsToComplete = monthlyBudget > 0 ? Math.ceil(remainingToSave / (monthlyBudget * 0.2)) : 0;
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-card rounded-lg border border-border p-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-muted-foreground">Meses p/ Completar</span>
          </div>
          <p className="text-lg font-bold text-blue-400">{monthsToComplete} meses</p>
          <p className="text-[10px] text-muted-foreground">guardando 20% da renda</p>
        </div>
      </div>

      {/* Impact Alert */}
      {remainingToSave > monthlyBudget && (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3">
          <p className="text-xs text-yellow-400">
            ⚠️ <strong>Impacto no Orçamento:</strong> Seus desejos representam {Math.round((remainingToSave / monthlyBudget) * 100)}% 
            do seu orçamento mensal. Considere priorizar ou parcelar.
          </p>
        </div>
      )}
      ...
      <div className="bg-card rounded-lg border border-border">
        ...
        {showForm && (
          <div className="p-3 border-b border-border bg-muted/30 space-y-2">
            ...
            <Button size="sm" onClick={addItem} className="h-7 text-xs">
              Salvar Item
            </Button>
          </div>
        )}
        ...
        <div className="divide-y divide-border">
          {sortedItems.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground">Adicione itens que você deseja comprar</p>
            </div>
          ) : (
                ...
                <div key={item.id} className="p-3 hover:bg-muted/30 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-[10px] px-1.5 py-0.5 rounded border ${priorityColors[item.priority]}`}>
                        ...
                        {item.link && (
                          <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-[10px] text-blue-400 hover:underline">
                            🔗 Link
                          </a>
                        )}
                      </div>
                  ...
                  <div className="mt-2 space-y-1">
                    <div className="flex items-center justify-between text-[10px]">
                      ...
                      <span className="text-green-400">
                        Guardado: R$ {item.savedAmount.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Input
                        ...
                        onBlur={(e) => updateSavedAmount(item.id, parseFloat(e.target.value) || 0)}
                        defaultValue={item.savedAmount}
                      />
