  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-card rounded-xl border border-border p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Calendar className="w-4 h-4 text-[hsl(var(--saude-green))]" />
            <span className="text-xs font-bold uppercase tracking-wider">Consultas</span>
          </div>
          <button onClick={() => setShowApptForm(!showApptForm)} className="text-[10px] px-3 py-1.5 rounded-lg bg-[hsl(var(--saude-green)/0.12)] text-[hsl(var(--saude-green))] font-bold border border-[hsl(var(--saude-green)/0.2)]">
          ...
          {showApptForm && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-3">
              <div className="grid gap-2 p-3 rounded-xl bg-muted">
                <Input value={newAppt.doctor} onChange={e => setNewAppt({ ...newAppt, doctor: e.target.value })} placeholder="Nome do médico" className="text-xs h-8" />
                <Input value={newAppt.specialty} onChange={e => setNewAppt({ ...newAppt, specialty: e.target.value })} placeholder="Especialidade" className="text-xs h-8" />
                <Input type="date" value={newAppt.date} onChange={e => setNewAppt({ ...newAppt, date: e.target.value })} className="text-xs h-8" />
                <Input value={newAppt.address} onChange={e => setNewAppt({ ...newAppt, address: e.target.value })} placeholder="Endereço" className="text-xs h-8" />
                ...
                <button onClick={addAppointment} className="py-2 rounded-lg bg-[hsl(var(--saude-green)/0.12)] text-[hsl(var(--saude-green))] text-xs font-bold">Salvar</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {upcomingAppts.length > 0 && (
          <div className="space-y-2 mb-3">
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Próximas</p>
            {upcomingAppts.map(a => (
              <div key={a.id} className="p-3 rounded-xl bg-[hsl(var(--saude-green)/0.06)] border border-[hsl(var(--saude-green)/0.15)]">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm font-bold text-foreground">{a.doctor}</p>
                    <p className="text-[11px] text-muted-foreground">{a.specialty}</p>
                    <p className="text-[11px] text-[hsl(var(--saude-green))] font-medium mt-1">
                ...
                {expandedAppt === a.id && a.questions && (
                  <div className="mt-2 p-2 rounded-lg bg-muted text-xs text-muted-foreground">{a.questions}</div>
                )}
              </div>
            ))}
          </div>
        ...
        {pastAppts.length > 0 && (
          <div className="space-y-2">
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Anteriores</p>
            {pastAppts.slice(0, 3).map(a => (
              <div key={a.id} className="flex items-center gap-3 p-2 rounded-lg bg-muted/50">
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium truncate text-foreground">{a.doctor} - {a.specialty}</p>
                  <p className="text-[10px] text-muted-foreground">{new Date(a.date + "T12:00:00").toLocaleDateString("pt-BR")}</p>
                </div>
                <button onClick={() => setAppointments(prev => prev.filter(x => x.id !== a.id))}>
      ...
      <div className="bg-card rounded-xl border border-border p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <FileText className="w-4 h-4 text-[hsl(var(--saude-blue))]" />
          ...
          <button onClick={() => examFileRef.current?.click()} disabled={uploading}
            className="text-[10px] px-3 py-1.5 rounded-lg bg-[hsl(var(--saude-blue)/0.12)] text-[hsl(var(--saude-blue))] font-bold border border-[hsl(var(--saude-blue)/0.2)]">
        ...
        {exams.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">Nenhum exame salvo</p>
        ) : (
          <div className="space-y-1.5">
            {exams.sort((a, b) => b.date.localeCompare(a.date)).map(e => (
              <div key={e.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-muted/50">
                <div className="w-8 h-8 rounded-lg bg-[hsl(var(--saude-blue)/0.12)] flex items-center justify-center text-[10px] font-bold text-[hsl(var(--saude-blue))]">{e.type}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium truncate text-foreground">{e.name}</p>
                  <p className="text-[10px] text-muted-foreground">{new Date(e.date + "T12:00:00").toLocaleDateString("pt-BR")}</p>
                </div>
                <a href={e.url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-[hsl(var(--saude-blue))] font-bold">Ver</a>
      ...
      <div className="bg-card rounded-xl border border-border p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <TrendingUp className="w-4 h-4 text-[hsl(var(--saude-green))]" />
            <span className="text-xs font-bold uppercase tracking-wider">Biomarcadores</span>
          </div>
          <button onClick={() => setShowBioForm(!showBioForm)} className="text-[10px] px-3 py-1.5 rounded-lg bg-[hsl(var(--saude-green)/0.12)] text-[hsl(var(--saude-green))] font-bold border border-[hsl(var(--saude-green)/0.2)]">
          ...
          {showBioForm && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-3">
              <div className="grid grid-cols-2 gap-2 p-3 rounded-xl bg-muted">
                <Input value={newBio.name} onChange={e => setNewBio({ ...newBio, name: e.target.value })} placeholder="Ex: Testosterona" className="col-span-2 text-xs h-8" />
                <Input value={newBio.unit} onChange={e => setNewBio({ ...newBio, unit: e.target.value })} placeholder="Unidade" className="text-xs h-8" />
                <div className="flex gap-1">
                  <Input value={newBio.refMin} onChange={e => setNewBio({ ...newBio, refMin: e.target.value })} placeholder="Ref Mín" className="text-xs h-8" />
                  <Input value={newBio.refMax} onChange={e => setNewBio({ ...newBio, refMax: e.target.value })} placeholder="Ref Máx" className="text-xs h-8" />
                </div>
                <button onClick={addBiomarker} className="col-span-2 py-2 rounded-lg bg-[hsl(var(--saude-green)/0.12)] text-[hsl(var(--saude-green))] text-xs font-bold">Salvar</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
